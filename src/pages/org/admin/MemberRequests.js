...
import { Snackbar, FormControlLabel, Switch, Button, Dialog, DialogActions, DialogTitle, Grid, makeStyles, Typography, TextField } from "@material-ui/core";
...
export default function Members({ match }) {
	...
	const [buttonEnabled, setButtonEnabled] = React.useState(org?.joinInstructions === null || org.joinInstructions.buttonEnabled)
		...
		<div className={classes.margin}>
			...
			<Grid container alignItems={"center"} spacing={2} className={classes.topBottomMargin}>
				<Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
					<TextField fullWidth multiline variant="outlined" label="Join Instructions" value={instructions} onChange={e => setInstructions(e.target.value)}/>
				</Grid>
				<Grid item xs={8} sm={8} md={4} lg={4} xl={4}>
					<FormControlLabel control={<Switch checked={buttonEnabled} onChange={e => setButtonEnabled(e.target.checked)}/>} label="Allow join through StuyActivities"/>
				</Grid>
				<Grid item xs={4} sm={4} md={2} lg={2} xl={2}>
					<Button style={{float: "right"}} color="primary" variant="contained" onClick={() => joinInstructionsMutation({variables: {orgId: org.id, instructions, buttonEnabled}})}>Set</Button>
				</Grid>
			</Grid>
